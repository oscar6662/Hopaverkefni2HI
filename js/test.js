  ...
  .then(data => {
    for(var i = 0; i < Object.keys(data.categories).length; i++) {
      ...
      for(var u = 0; u < Object.keys(data.categories[i].videos).length; u++) {
        ...
        <a href="pages/myndband${data.videos[data.categories[i].videos[u] - 1].id}.html"><div class="myndband${data.videos[data.categories[i].videos[u] - 1].id} col col-4 col-sm-12 myndband">
          <div class="myndband-poster">
            <img src="${data.videos[data.categories[i].videos[u] - 1].poster}">
            <div class="myndband-poster-time"><p>${minutes + ':' + ('0' + seconds).slice(-2)}</p></div
            </div>
